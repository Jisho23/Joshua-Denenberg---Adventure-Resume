...
exports.input = function(input, gameID) {
  ...
  if (game) {
    ...
    try {
      try {
        debug(
          '---Attempting to run cartridge command "' + command.action + '"'
        );
        returnString = eval(
...
function loadCartridge(gameID, gameName) {
  ...
  try {
    ...
    return (
      games[gameID].gameData.introText +
      "\n" +
  ...
  help: function(game, command) {
    return {
      message:
        ...
        "\n  -If you want to see what your prior action was, input 'prior'."
...
function exitsToString(exitsObject) {
  ...
  for (i = 0; i < visibleExits.length; ++i) {
    ...
    if (i === visibleExits.length - 2) {
      returnString = returnString.concat(" or ");
...
function interact(game, interaction, subject) {
  try {
    return (message = getCurrentLocation(game).items[subject].interactions[
      interaction
    ]);
  } catch (error) {
